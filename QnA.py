        ...
        while True:
            with sr.Microphone() as source:
                ...
                try:
                    ...
                    if "repeat option one only" in answer.lower():
                        engine.say(options[0])
                        engine.runAndWait()
                        continue  # Ask the question again
                     #if user wants to repeat only option 2
                    if "repeat option two only" in answer.lower():
                        engine.say(options[1])
                        engine.runAndWait()
                        continue  # Ask the question again
                    if "repeat option three only" in answer.lower():
                        engine.say(options[2])
                        engine.runAndWait()
                        continue  # Ask the question again
                    if "repeat option four only" in answer.lower():
                        engine.say(options[3])
                        engine.runAndWait()
                        continue  # Ask the question again
                    if "next question" in answer.lower():
                        #engine.say(options[1])
                        #engine.runAndWait()
                        #nextQues=True
                        break  # Ask the question again




                    # Check if user wants to end the exam
                    if "end exam" in answer.lower():
